...
<!-- Published URL
https://www.servicenow.com/community/automation-engine-blog/how-to-migrate-from-scripted-integration-to-integration-hub-part/ba-p/2680006
-->
...
- [Blog 2](***URL***) This blog on replacing scripted web service with a custom spoke action
...
> **Note**: The Connection and Credential record itself will not be captured in your deployment method of choice. This is intentional as the connection URL and credential used most likely will vary on your different ServiceNow instances. The development environment will connect to a target development system whereas the production connects to a production system. We don't want to overwrite these connection details with every deployment. This also means when you install the prepared applications from the Git repositories, you will need to create the Connection & Credential record on your instance.

With the alias and connection created, we can move forward and create the Integration Hub Action. Best is to open **Process Automation -> Flow Designer**. If you prefer, you can also do that from Developer Studio, it will launch Flow Designer in a separate window. For every API call to an external system we need a bespoke action to handle it. In our case we need two to replace the calls to *Search by Title* and *Get by Title*.

### Create Action Get by Title
...
With that you are ready for your first test of this particular step. Click on **TEST** next to *Publish*, provide a movie title and run your action. It will connect to OMDb API and fetch details from it in a JSON format. So this was at least not more difficult than it was in the traditional way, right? Let's get to the real power now.

To parse the response body into actionable data objects we add a step called **JSON Parser step**. Like before, click on the **Plus (+)** and find this step.
...
With that the action as such is complete. Last piece is defining the outputs for the whole action. Basically what this action returns to whomever calls it. Create two output parameters of type Object and one of type String, then map them as shown below. Notice, as you map the data pills, Flow Designer will inspect the object and generate all elements automatically for you. You can also get creative and define your own object and structure mapping to elements within rather then exposing the full objects received, similar to the JSON Parser step.
...
This code is merely the same complexity as our old style Web Service Call, but it does leverage the new Integration Hub Spoke. Check out the full script as both API calls have been replace with Integration Hub calls on the git repository in branch _Blog2_add_spoke_.

### Testing

For testing purposes it is a good idea to [activate Flow reporting](https://docs.servicenow.com/bundle/vancouver-build-workflows/page/administer/flow-designer/task/enable-flow-reporting.html). With that activated, use the service catalog item 'Classic Find a Movie'. Once you found a movie and submit the record producer you can see the executions of your actions at **Process Automation -> Flow Administration -> Today's Executions**.

![IntegrationHub Executions](blog2_images/ihub_executions.png)

It is also possible to drill into these executions and see all details about them, depending on your reporting settings for Flow Designer.

### Summary

I hope this blog demonstrated that it is very easy to leverage the newer technology of Integration Hub to build your custom integration. There is certainly not more effort then before, but you gain a lot more flexibilty and additional features like the JSON Parser step. In the last blog post I will summarize additional benefits of using Integration Hub.